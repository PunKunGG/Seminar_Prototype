...
- วิเคราะห์พฤติกรรมจากภาพวิดีโอ เช่น ตั้งใจเรียน หลับ ก้มหน้า ใช้โทรศัพท์ สงสัยใช้โทรศัพท์ ยกมือ และยืน/ลุกจากที่นั่ง
- แยกการใช้โทรศัพท์ที่ยืนยันจาก object detector ออกจากท่ามือที่เพียงน่าสงสัย และใช้จอคอมเป็นบริบทประกอบการมองลง
- ติดตามตำแหน่งแบบไม่ระบุตัวตนด้วย `ID 1`, `ID 2`, ... ภายในแต่ละรอบวิเคราะห์ และเรียกคืน ID เมื่อกลับมายังตำแหน่งเดิม
- นับเหตุการณ์และเวลาของพฤติกรรมแยกราย ID โดยมีช่วงยืนยันพฤติกรรมเพื่อลดการนับซ้ำทุกเฟรม
- เก็บภาพอ้างอิงของแต่ละ ID และภาพเหตุการณ์เมื่อพฤติกรรมสำคัญผ่านเกณฑ์ยืนยัน โดยรายงานแสดงภาพตัวแทนหนึ่งภาพต่อ ID
- สรุปเปอร์เซ็นต์ความตั้งใจและช่วงเวลาเริ่ม-สิ้นสุดของพฤติกรรม แบ่งรายงานเป็นช่วงละ 1 ชั่วโมง
...
- รายงานแสดงภาพตัวแทนหนึ่งภาพต่อ Position ID พร้อมพฤติกรรมในภาพ ส่วนภาพเหตุการณ์อื่นยังเก็บเป็นข้อมูลของรอบวิเคราะห์
- PDF ใช้ภาพ JPEG คุณภาพเอกสารและ render ประมาณ 216 DPI เพื่อลดขนาดไฟล์ โดย CSV/Excel/JSON ไม่ได้รับผลกระทบ
...
| `CLASSMOOD_ANNOTATED_STREAM_FPS` | `5` | จำนวนครั้งสูงสุดต่อวินาทีที่ AI worker วิเคราะห์ภาพและอัปเดตกรอบ จำกัดสูงสุดที่ 5 |
| `CLASSMOOD_CONTEXT_DETECTION_ENABLED` | เปิด | ตรวจโทรศัพท์และจอเป็นบริบทประกอบ pose ปิดได้เมื่อต้องการลดภาระเครื่อง |
| `CLASSMOOD_CONTEXT_DETECTION_INTERVAL` | `5` | ระยะห่างเป็นวินาทีระหว่างการตรวจวัตถุแต่ละครั้ง โดยใช้ผลล่าสุดระหว่างช่วง |
| `CLASSMOOD_CONTEXT_DETECTION_CONFIDENCE` | `0.20` | confidence ขั้นต่ำของโทรศัพท์และจอ |
| `CLASSMOOD_CONTEXT_DETECTION_IMAGE_SIZE` | `960` | ขนาดภาพสำหรับ object detector ค่าสูงช่วยวัตถุเล็กแต่ใช้ทรัพยากรมากขึ้น |
...
| `CLASSMOOD_TRACK_POSITION_ENABLED` | เปิด | เรียกคืน Position ID เมื่อกลับมายังตำแหน่งเดิม ปิดได้สำหรับกล้องที่เคลื่อนที่หรือไม่มีที่นั่งคงที่ |
| `CLASSMOOD_TRACK_POSITION_MEMORY_SECONDS` | `21600` | เวลาที่จำตำแหน่งเดิมเพื่อเรียกคืน Position ID ภายในรอบวิเคราะห์ |
| `CLASSMOOD_TRACK_POSITION_MAX_DISTANCE` | `0.45` | ระยะตำแหน่งสัมพัทธ์สูงสุดสำหรับเรียกคืน ID เดิม ค่าน้อยเข้มงวดกว่า |
...
| `CLASSMOOD_EVIDENCE_BEHAVIORS` | `sleeping,looking_down,phone_use,phone_suspected,hand_raised,standing` | พฤติกรรมที่ต้องเก็บภาพเมื่อเริ่มเหตุการณ์ |
...
ID เป็นรหัสตำแหน่งชั่วคราวของรอบวิเคราะห์ ไม่ใช่รหัสนักศึกษา ระบบจับคู่จากการเคลื่อนไหวและตำแหน่งช่วงล่างของกรอบ เมื่อกลับมายังตำแหน่งเดิมระบบจะพยายามคืน ID เดิม วิธีนี้ต้องใช้กล้องมุมคงที่และผู้เรียนไม่สลับที่นั่ง หากมีการสลับที่ ระบบไม่สามารถยืนยันว่าเป็นคนเดิมได้โดยไม่ใช้ข้อมูลชีวมิติ

### การแยกก้มหน้า จอคอม และโทรศัพท์

ระบบจะรายงาน `ใช้โทรศัพท์` เมื่อ object detector พบโทรศัพท์ใกล้กรอบบุคคล หากภาพเต็มไม่พบแต่ท่ามือเข้าข่าย ระบบจะตรวจซ้ำจากภาพครอปเฉพาะบุคคลในรอบตรวจบริบท และรายงาน `สงสัยใช้โทรศัพท์` เมื่อเห็นท่ามือสองข้างบรรจบต่ำกว่าไหล่ต่อเนื่องอย่างน้อย 4 วินาทีโดยยังไม่เห็นตัวโทรศัพท์ สถานะสงสัยไม่ถูกนับรวมในความตั้งใจและแยกจากหลักฐานที่ยืนยันแล้ว จอคอมที่ตรวจพบใกล้ระดับสายตาจะใช้เป็นบริบทประกอบการมองลง ทั้งนี้ หากโทรศัพท์ถูกบังทั้งหมด ระบบยืนยันการใช้โทรศัพท์จากภาพไม่ได้ และการพิมพ์หรือจับวัตถุอื่นอาจมีท่าคล้ายกัน จึงควรตรวจภาพหลักฐานในรายงานก่อนสรุปผล
