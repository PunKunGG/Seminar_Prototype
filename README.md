...
- สรุปเปอร์เซ็นต์ความตั้งใจ จำนวนครั้ง ระยะเวลารวม และช่วงเริ่ม-สิ้นสุดของพฤติกรรมราย Position ID
- แยกข้อมูลตามห้อง รายวิชา และคาบเรียนด้วย SQLite; เมื่อกำหนด server key จะสำรองผลรอบที่จบแล้วและภาพหลักฐานไป Supabase
...
ใส่ค่า `SUPABASE_URL` และ `SUPABASE_PUBLISHABLE_KEY` จาก Supabase เพื่อเปิดใช้ล็อกอิน
คีย์นี้ไม่ได้ทำให้ผลวิเคราะห์ถูกเก็บใน Supabase โดยอัตโนมัติ: การสำรองผลและภาพหลักฐานต้องกำหนด `SUPABASE_SECRET_KEY` (หรือ `SUPABASE_SERVICE_ROLE_KEY`) ให้ **Python server เท่านั้น** ผ่าน environment หรือ `.env` ในเครื่องที่ Git ignore ไว้ ห้ามใส่คีย์นี้ใน JavaScript, commit ลง repo หรือส่งให้ผู้ใช้เว็บ
schema ใน `supabase/migrations/` และ private bucket `class-evidence` ต้องถูกสร้างในโปรเจกต์ปลายทางก่อน หากไม่ได้กำหนด server key ประวัติยังเปิดได้จาก SQLite บนเครื่องเดิม แต่ไม่ย้ายตามไปยังเครื่องอื่น
...
2. เลือกเปิดรายงานรอบที่ผ่านมา หรือสร้างรอบใหม่โดยตั้งชื่อ รายวิชา ห้อง และช่วงสรุปผล (15-300 วินาที; เริ่มต้น 30)
3. กด `เริ่มวิเคราะห์`
4. เลือกแท็บแหล่งภาพ: วิดีโอต้องระบุวันที่และเวลาเริ่มบันทึกก่อนอัปโหลด `.mp4`, `.avi`, `.mov`, `.mkv` หรือ `.webm`; เว็บแคมใช้ปุ่มเริ่ม/หยุดและเวลาจริงของเครื่องที่รัน Python
5. ระบบจะแสดงภาพวิเคราะห์ กราฟ และตารางราย ID แบบ real time
6. เมื่อคลิปจบหรือกดหยุดเว็บแคม ระบบเปิดรายงานให้อัตโนมัติ จากนั้นเปิดดูซ้ำได้ที่หน้าเริ่มต้นโดยไม่ต้องดาวน์โหลด
7. PDF เป็นตัวเลือก ไม่จำเป็นต้องส่งออกไฟล์เพื่อเก็บรายงานในเว็บ
...
- ค่าเริ่มต้นวิเคราะห์ช่วง 15 วินาทีแรกของทุก 30 วินาทีของคลิป ปรับระยะห่างได้ต่อรอบ (ขั้นต่ำ 15 วินาที)
- ใช้ 2 เฟรมต่อวินาทีภายในช่วงที่เลือก; เลือกทุก 30 วินาทีใช้ทรัพยากรมากกว่าทุก 1 นาที
- รวมผลหลายเฟรมเป็นสถิติหนึ่งจุดต่อช่วงที่เลือก
- แสดงเปอร์เซ็นต์และตำแหน่งเวลาในคลิประหว่างประมวลผล
- เปิดรายงานอัตโนมัติเมื่อประมวลผลครบทุกช่วงเหมือนคลิปสั้น
- รายงานบนหน้าเว็บและ PDF สรุปเป็นช่วงละ 5 นาทีโดยไม่แสดงวินาทีในตารางหลัก; ข้อมูลเหตุการณ์ละเอียดบนเว็บและใน JSON/CSV ยังอยู่
- ผลรายบุคคลสรุปรวมตลอดคาบ ส่วนตารางเหตุการณ์ระบุเวลาเริ่ม สิ้นสุด และระยะเวลาของแต่ละช่วง
...
ระบบตรวจบุคคลและ keypoint 17 จุดด้วย YOLOv8 Pose แล้วใช้กฎหลายสัญญาณร่วมกัน ค่า keypoint ต้องมี confidence อย่างน้อย `0.40` จึงนำมาใช้ ค่าเริ่มต้นสรุปสถิติ real-time ทุก `30 วินาที` ส่วนคลิปยาววิเคราะห์ช่วงละ `15 วินาที` ในทุก 30 วินาทีที่เลือก เวลายืนยันด้านล่างเป็นระยะที่ผลต้องต่อเนื่องก่อนเปลี่ยนสถานะของบุคคล ไม่ใช่รอบการสรุปรายงาน

| พฤติกรรม | สิ่งที่วัดและเกณฑ์หลัก | เวลายืนยัน | ข้อจำกัด |
| --- | --- | ---: | --- |
| ตั้งใจเรียน | ระดับจมูกเทียบไหล่ ความตั้งตรงของลำตัว ระยะตา การเห็นหู และบริบทจอ คะแนนอย่างน้อย 2, confidence อย่างน้อย 40% และชนะอันดับถัดไปอย่างน้อย 0.5 | 1.0 วินาที | เป็นการอนุมานจากท่าทาง ไม่ยืนยันว่ากำลังอ่านหรือเข้าใจบทเรียน |
| หลับ | จมูกต่ำใกล้แนวไหล่ร่วมกับลำตัวพับหรือเอนมาก | 8.0 วินาที | ไม่ตรวจการลืมตา/หลับตา อาจสับสนกับการก้มเก็บของ เอนตัว หรือถูกบัง |
| ก้มหน้า | ระดับจมูกเทียบกึ่งกลางไหล่และแนวลำตัว โดยจอใกล้ระดับสายตาช่วยลดคะแนนก้มหน้า | 3.0 วินาที | แยกไม่ได้แน่นอนว่ากำลังจดบันทึก อ่านเอกสาร หรือทำกิจกรรมอื่น |
| ใช้โทรศัพท์ | YOLO ตรวจวัตถุโทรศัพท์ใกล้บุคคลทั้งจากภาพเต็มและภาพครอปตรวจซ้ำ | 3.0 วินาที | โทรศัพท์เล็ก เบลอ ถูกมือบัง หรือมุมไม่ชัดอาจตรวจไม่พบ |
| สงสัยใช้โทรศัพท์ | ข้อมืออยู่ใกล้กัน ต่ำกว่าไหล่ และปลายแขนบรรจบ แต่ยังไม่พบวัตถุโทรศัพท์ | 4.0 วินาที | ไม่ใช่ผลยืนยัน อาจเป็นการพิมพ์ จับวัตถุ หรือประสานมือ |
| ยกมือ | ข้อมือเหนือใบหน้า ศอกใกล้หรือเหนือไหล่ และข้อมือสูงกว่าศอก | 1.2 วินาที | ท่าบังหน้า โบกมือ หรือมุมด้านข้างอาจให้ผลคล้ายกัน |
| ยืน/ลุก | แนวไหล่ สะโพก เข่า ข้อเท้า ความยาวลำตัว และมุมเข่าอย่างน้อย 150 องศา | 1.0 วินาที | ต้องเห็นช่วงล่างเพียงพอ โต๊ะหรือบุคคลอื่นบังขาอาจทำให้ไม่พบ |
| ไม่ชัดเจน | keypoint ที่เชื่อถือได้น้อยกว่า 6 จุด ข้อมูลขัดแย้ง หรือคะแนนสูสีกัน | 1.5 วินาที | หมายถึงหลักฐานไม่พอสรุป ไม่ได้หมายถึงไม่มีพฤติกรรม |

อัตราความตั้งใจราย Position ID คำนวณจาก `(เวลาตั้งใจเรียน + เวลายกมือ) / เวลาที่วิเคราะห์บุคคลทั้งหมด x 100` ค่า confidence ที่แสดงเป็นสัดส่วนคะแนนจากกฎ ไม่ใช่ความน่าจะเป็นที่ผ่านการสอบเทียบ ระบบไม่ใช้ใบหน้า เสียง เนื้อหาบนจอ หรือข้อมูลชีวมิติในการตัดสิน
...
เมื่อเปิด Supabase archive ระบบส่งเฉพาะข้อมูลรายงาน, ผลราย ID และภาพหลักฐานของรอบที่จบแล้วไปคลาวด์ ไม่ส่งคลิปต้นฉบับ; วิดีโออัปโหลดยังอยู่ในเครื่องที่รัน server การลบไฟล์ SQLite/ภาพในเครื่องก่อนเห็นสถานะ `บันทึกบนคลาวด์` อาจทำให้รายงานหรือภาพหาย
รอบ SQLite เก่าที่ไม่มีเจ้าของจะไม่แสดงแก่บัญชีที่ล็อกอินเพื่อไม่ให้ข้อมูลข้ามผู้ใช้; ต้องตรวจสอบเจ้าของก่อนย้ายข้อมูลเดิม
...
| `/api/sessions` | GET | ประวัติรอบของผู้ใช้ที่ล็อกอิน รวมรอบจากเครื่องและ Supabase |
| `/api/videos` | POST | อัปโหลดไฟล์วิดีโอ |
| `/api/sources/<session_id>/<source_id>` | POST/DELETE | เริ่มหรือหยุดแหล่งภาพ; วิดีโอต้องส่ง `recording_start` ที่มี timezone |
...
| `/api/evidence/<session_id>/<filename>` | GET | ภาพหลักฐานของรอบที่เป็นเจ้าของ จากเครื่องหรือ private Supabase Storage |
...
| `CLASSMOOD_MAX_HISTORY` | `1200` | จำนวนจุดสถิติสูงสุดต่อรอบ รองรับมากกว่า 8 ชั่วโมงเมื่อสรุปทุก 30 วินาที |
| `CLASSMOOD_LONG_VIDEO_THRESHOLD_SECONDS` | `1800` | ความยาวขั้นต่ำที่เปิดโหมดคลิปยาว |
| `CLASSMOOD_LONG_VIDEO_SAMPLE_INTERVAL_SECONDS` | `30` | ค่าเริ่มต้นระยะห่างระหว่างช่วงตัวอย่าง (หน้าเว็บส่งค่าของรอบมาแทนได้) |
| `CLASSMOOD_LONG_VIDEO_SAMPLE_WINDOW_SECONDS` | `15` | ความยาวของช่วงที่นำมาวิเคราะห์ในแต่ละรอบ |
...
| `CLASSMOOD_REALTIME_STATS_INTERVAL` | `30` | ค่าเริ่มต้นระยะห่างการบันทึกสถิติรวม (หน้าเว็บส่งค่าของรอบมาแทนได้) |
| `SUPABASE_SECRET_KEY` | ไม่ตั้ง | คีย์ลับเฉพาะ Python server เพื่อสำรองรายงานและภาพ; ใช้ `SUPABASE_SERVICE_ROLE_KEY` แทนได้ |
